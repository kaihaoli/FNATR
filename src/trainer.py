...
from model import BiencoderOutput, BiencoderModel
from utils import AverageMeter


def _unpack_qp(inputs: Dict[str, torch.Tensor]) -> Tuple:
    q_prefix, d_prefix, kd_labels_key, additional_d_prefix = 'q_', 'd_', 'kd_labels', 'additional_d_'
    ...
    additional_doc_batch_dict = {k[len(additional_d_prefix):]: v for k, v in inputs.items() if k.startswith(additional_d_prefix)}
    ...
    if not doc_batch_dict:
        doc_batch_dict = None
    if not additional_doc_batch_dict:
        additional_doc_batch_dict = None

    return query_batch_dict, doc_batch_dict, additional_doc_batch_dict


class BiencoderTrainer(Trainer):
    def __init__(self, *pargs, batch_sampler=None, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        query, passage, additional_passage = _unpack_qp(inputs)
        outputs: BiencoderOutput = model(query=query, passage=passage, additional_passage=additional_passage)
        loss = outputs.loss
