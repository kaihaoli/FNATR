...
from data_utils import load_queries, load_qrels, load_predictions, save_preds_to_output_format
from model import BiencoderModelForInference, BiencoderOutput
...
def _worker_encode_queries(gpu_idx: int) -> Tuple:
    ...
    if args.data_name == 'msmarco':
        suffix = '_queries.tsv'
    elif args.data_name == 'esci':
        suffix = '.jsonl'
    else:
        ValueError(f'invalid data name :{data_name}') 

    query_id_to_text = load_queries(path=os.path.join(args.data_dir, '{}{}'.format(args.search_split, suffix)))
...
def _compute_and_save_metrics(worker_cnt: int):
    ...
    if args.data_name == 'msmarco':
        out_suffix = '.msmarco.txt'
        qrels_suffix = '_qrels.txt'
    elif args.data_name == 'esci':
        out_suffix = '.esci.txt'
        qrels_suffix = '.jsonl'
    else:
        ValueError(f'invalid data name :{data_name}')
    out_path = os.path.join(args.search_out_dir, '{}{}'.format(args.search_split, out_suffix))
    save_preds_to_output_format(preds, out_path)
    logger.info('Merge done: save {} predictions to {}'.format(len(preds), out_path))

    path_qrels = os.path.join(args.data_dir, '{}{}'.format(args.search_split, qrels_suffix))
