...
class BiencoderCollator(DataCollatorWithPadding):
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> BatchEncoding:
        q_prefix, d_prefix, ad_prefix = 'q_', 'd_', 'additional_d_'
        ...
        if [k for k, v in features[0].items() if k.startswith(ad_prefix)]:
            additional_doc_examples = _unpack_doc_values(
                [{k[len('additional_d_'):]: v for k, v in f.items() if k.startswith('additional_d_')} for f in features])
        else:
            additional_doc_examples = []
        ...
        if additional_doc_examples:
            a_collated = self.tokenizer.pad(
                ...
                return_tensors=self.return_tensors)
            for k in a_collated:
                q_collated['additional_d_' + k] = a_collated[k]
